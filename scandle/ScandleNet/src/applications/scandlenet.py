'''
scandlenet.py: contains run function for spectralnet
'''
...
from ScandleNet.src.core import train
from ScandleNet.src.core import costs
from ScandleNet.src.core import networks
from ScandleNet.src.core.layer import stack_layers
from ScandleNet.src.core.util import get_scale, print_accuracy, get_cluster_sols, LearningHandler, make_layer_list, train_gen, get_y_preds
...
def run_net(data, params):
    ...
    x_scandlenet = spectral_net.predict(x)

    # get accuracy and nmi
    kmeans_assignments, km = get_cluster_sols(x_scandlenet, ClusterClass=KMeans, n_clusters=params['n_clusters'], init_args={'n_init':10})
    y_scandlenet, _ = get_y_preds(kmeans_assignments, y, params['n_clusters'])
    ...
    if params['generalization_metrics']:
        x_scandlenet_train = spectral_net.predict(x_train_unlabeled)
        x_scandlenet_test = spectral_net.predict(x_test)
        km_train = KMeans(n_clusters=params['n_clusters']).fit(x_scandlenet_train)
        from scipy.spatial.distance import cdist
        dist_mat = cdist(x_scandlenet_test, km_train.cluster_centers_)
    ...
    return x_scandlenet, y_scandlenet
