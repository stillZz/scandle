...
from ScandleNet.src.core import pairs
...
def load_data(params):
    ...
    if params['dset'] == 'reuters':
        with h5py.File('../../data/reuters/reutersidf_total.h5', 'r') as f:
            ...
            x_train, x_test = x[:], x[n_train:]
            y_train, y_test = y[:], y[n_train:]
    elif params['dset'] == 'mnist':
        x_train, x_test, y_train, y_test = get_mnist()
    elif params['dset'] == 'fashion_mnist':
    ...
    elif params['dset'] == 'other':
        x_train, x_test, y_train, y_test = generate_data(params.get('n'), params.get('noise_sig'), params.get('train_set_fraction'))
        x_train, x_test = pre_process(x_train, x_test, params.get('standardize'))
    else:
...
def embed_data(x, dset):
    ...
    json_path = '../pretrain_weights/ae_fashion_mnist.json'
    weights_path = '../pretrain_weights/ae__fashion_mnist_weights.h5'
...
def generate_data(n=1200, noise_sigma=0.01, train_set_fraction=1.):
    np.random.seed(0)
    1
    x,y = datasets.make_circles(n_samples=n, factor=.5,noise=noise_sigma)
    #2
    # x,y=datasets.make_blobs(n_samples=n, cluster_std=[1.0, 2.5, 0.5],random_state=170)
    #3
    # random_state = 170
