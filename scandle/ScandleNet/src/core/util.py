...
from ScandleNet.src.core import costs as cf
...
class LearningHandler(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # K.get_session().run(tf.variables_initializer(self.lr_tensor))
...
def make_random_Y(x,y):
    ...
    return l
sess = tf.Session()
print(len(make_random_Y(1024,10)))
indices =make_random_Y(1024,10)
Y = tf.one_hot(indices,10)
print(Y)
print(sess.run(Y))
...
def acc(y_true, y_pred):
    ...
    return sum([w[i, j] for i, j in b]) * 1.0 / y_pred.size