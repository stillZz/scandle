...
from ScandleNet.src.core.util import make_batches
...
def predict_finally(predict_var,Q, x_unlabeled, inputs, y_true, batch_sizes,
        x_labeled=None, y_labeled=None):
    '''
    Evaluates predict_var, batchwise, over all points in x_unlabeled
    and x_labeled.
    ...
    for i, (batch_start, batch_end) in enumerate(batches):
        ...
        y_pred_batch = np.asarray(K.get_session().run(predict_var, feed_dict=feed_dict))
        y_pred_batch = tf.argmax(tf.matmul(y_pred_batch, Q), 1)
        # tf.Session().run(tf.global_variables_initializer())
        y_preds.append(y_pred_batch)

        # print(K.get_session().run(Q))
        # print(sess.run(tf.matmul(tf.transpose(Q), Q)))

    # print(sess.run(tf.matmul(tf.transpose(Q),Q)))

    # print(sess.run(predict_var))

    if len(y_preds[0].shape):
        return tf.concat(y_preds,0)
    else:
        return tf.sum(y_preds)
    print(K.get_session().run(y_preds))
