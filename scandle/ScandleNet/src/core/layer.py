...
# import keras.layers
# sess = tf.Session()
# # x = tf.constant([[1,0,0],[0,3,0],[0,0,4]],dtype =tf.float32)
# print("------------------")
# A = tf.Variable([1,2,3])
# print(A)
# x = tf.constant([[1,2,3],[0,5,6],[7,8,0]],dtype=tf.float32)
# ortho_weights = orthonorm_op(x)
# print(ortho_weights)
# print('------------------------')
# print(sess.run(ortho_weights))
# print(sess.run(K.dot(x,ortho_weights)))
# d = x.get_shape().as_list()[-1]
# print(d)
# ortho_weights_store = K.variable(np.zeros((d,d)))
# # sess.run(tf.global_variables_initializer())
# # print(sess.run(ortho_weights_store))
# # print(sess.run(K.dot(x, ortho_weights)))
# print('-------------------------------------------------')
# ortho_weights_update = tf.assign(ortho_weights_store, ortho_weights, name='ortho_weights_update')
# print(sess.run(ortho_weights_update))
# # l = Lambda(lambda x: K.in_train_phase(K.dot(x, ortho_weights), K.dot(x, ortho_weights_store)), name=None)
# l = Lambda(lambda x:K.dot(x, ortho_weights), name=None)
# print(sess.run(l.weights))
# l.add_update(ortho_weights_update)
# print(sess.run(l.weights))
# a = l(x)
# sess.run(tf.global_variables_initializer())
# print(sess.run(a))
# # print(l(keras.layers.Dense(3, activation=None)(x)))
# # print(sess.run(x))
# # l = Orthonorm(x)
